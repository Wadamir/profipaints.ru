...
$title    = get_theme_mod('profipaints_about_title', esc_html__('About Us', 'profipaints'));
$subtitle = get_theme_mod('profipaints_about_subtitle', esc_html__('Section subtitle', 'profipaints'));
$desc     = wp_kses_post(get_theme_mod('profipaints_about_desc'));
if (profipaints_is_selective_refresh()) {
    $disable = false;
}
// Get data
$page_ids = profipaints_get_section_about_data();
$content_source = get_theme_mod('profipaints_about_content_source');
...
<?php if (!$disable) { ?>
    <?php if (!profipaints_is_selective_refresh()) { ?>
        <section id="<?php if ($id != '') {
                            echo esc_attr($id);
                        }; ?>" <?php do_action('profipaints_section_atts', 'about'); ?> class="<?php echo esc_attr(apply_filters('profipaints_section_class', 'section-about section-meta section-padding onepage-section', 'about')); ?>">
        <?php } ?>
        ...
        <div class="<?php echo esc_attr(apply_filters('profipaints_section_container_class', 'container', 'about')); ?>">
            ...
            <div class="row">
                ...
                if (is_array($page_ids) && !empty($page_ids)) {
                    ...
                    // Layout columns
                    $_layout = absint(get_theme_mod('profipaints_about_layout', 3));
                    if ($n > $_layout) {
                        $num_col = $_layout;
