 ...
 * Class Profipaints_Dots_Navigation
 *
 * @since 2.1.0
 */
class Profipaints_Dots_Navigation
{
    ...
    private $key = 'profipaints_sections_nav_';

    /**
     * Get instance
     *
     * @return null|Profipaints_Dots_Navigation
     */
    ...
    function get_sections()
    {
         ...
         * @since 2.1.1
         */
        $new = Profipaints_Config::get_sections();

        // Filter to add more custom sections here
        return apply_filters('profipaints_sections_navigation_get_sections', $new);
    }
    ...
    function add_customize($wp_customize, $section_id)
    {
        ...
        $wp_customize->add_control(
            $this->get_name('__enable'),
            array(
                'label'       => __('Enable in section navigation', 'profipaints'),
                'section'     => $section_id,
                'type'        => 'checkbox',
            )
        );
        ...
        $wp_customize->add_control(
            $this->get_name('__disable_mobile'),
            array(
                'label'       => __('Disable the section navigation on mobile', 'profipaints'),
                'section'     => $section_id,
                'type'        => 'checkbox',
            )
        );
        ...
        $wp_customize->add_control(
            $this->get_name('__enable_label'),
            array(
                'label'       => __('Enable navigation labels', 'profipaints'),
                'description'       => __('By default navigation label is section title.', 'profipaints'),
                'section'     => $section_id,
                'type'        => 'checkbox',
            )
        );
        ...
        $wp_customize->add_control(
            new WP_Customize_Color_Control(
                ...
                $this->get_name('__color'),
                array(
                    'label'       => esc_html__('Dots color', 'profipaints'),
                    'section'     => $section_id,
                    'description' => '',
                )
            )
        ...
        $wp_customize->add_control(
            new WP_Customize_Color_Control(
                ...
                $this->get_name('__color2'),
                array(
                    'label'       => esc_html__('Dots inverse color', 'profipaints'),
                    'section'     => $section_id,
                    'description' => '',
                )
            )
        ...
        $wp_customize->add_control(
            new ProfiPaints_Alpha_Color_Control(
                ...
                $this->get_name('__label_bg'),
                array(
                    'label'       => esc_html__('Label Background', 'profipaints'),
                    'section'     => $section_id,
                    'description' => '',
                )
            )
        ...
        $wp_customize->add_control(
            new ProfiPaints_Alpha_Color_Control(
                ...
                $this->get_name('__label_color'),
                array(
                    'label'       => esc_html__('Label Color', 'profipaints'),
                    'section'     => $section_id,
                    'description' => '',
                )
            )
        ...
        foreach ($this->get_sections() as $id => $args) {
             ...
             * @since 2.1.1
             */
            if (Profipaints_Config::is_section_active($id)) {
                ...
                $wp_customize->add_control(
                    new ProfiPaints_Misc_Control(
                        $wp_customize,
                        $id . '_em',
                        array(
                            'type'        => 'custom_message',
                            'section'     => $section_id,
                            'description' => '<div class="profipaints-c-heading">' . esc_html($args['label']) . '</div>',
                        )
                    )
                );
                ...
                $wp_customize->add_control(
                    $name . '_inverse',
                    array(
                        'label'   => __('Inverse dots color', 'profipaints'),
                        'section' => $section_id,
                        'type'    => 'checkbox',
                    )
                );
                ...
                $wp_customize->add_control(
                    $name . '_label',
                    array(
                        'label'   => __('Custom navigation label', 'profipaints'),
                        'section' => $section_id,
                    )
                );
    ...
    function get_settings()
    {
        ...
        $data = apply_filters('profipaints_dots_navigation_get_settings', false);
        if ($data) {
            return $data;
        }
        ...
        foreach ($sections as $id => $args) {
            ...
            if (Profipaints_Config::is_section_active($id)) {
                if (
                    !get_theme_mod('profipaints_' . $id . '_disable', false)
                    ...
                    $el_id = sanitize_title(get_theme_mod('profipaints_' . $id . '_id', $translated_id, false));
                    if (!$el_id) {
                        $el_id = $id;
                    }
                    $data[$el_id] = array(
                        ...
                        'title'   => get_theme_mod('profipaints_' . $id . '_title', $args['title']),
                    );
    ...
    function custom_style($code = false)
    {
        if (get_theme_mod($this->get_name('__enable'), false)) {
            ...
            $bg_color = profipaints_sanitize_color_alpha($bg);
            ...
            $color = profipaints_sanitize_color_alpha($color);
    ...
    function init()
    {
        ...
        add_filter('profipaints_custom_css', array($this, 'custom_style'));
    }
}

Profipaints_Dots_Navigation::get_instance()->init();
