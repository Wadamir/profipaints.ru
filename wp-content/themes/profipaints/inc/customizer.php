 ...
 * @return string
 */
function profipaints_add_upsell_for_section($wp_customize, $section_id)
{
    if (apply_filters('profipaints_add_upsell_for_section', true, $section_id)) {
 ...
 */
function profipaints_customize_register($wp_customize)
{
     ...
     */
    do_action('profipaints_customize_before_register', $wp_customize);


    $pages  =  get_pages();
    $option_pages = array();
    $option_pages[0] = esc_html__('Select page', 'profipaints');
    foreach ($pages as $p) {
    ...
    $option_users[0] = esc_html__('Select member', 'profipaints');
    ...
    foreach ($sections as $key => $section) {
        ...
        if (Profipaints_Config::is_section_active($key)) {
	...
	*/
     ...
     * Hook to add other customize
     */
    do_action('profipaints_customize_after_register', $wp_customize);
...
add_action('customize_register', 'profipaints_customize_register');
/**
 * Selective refresh
 */
 ...
 */
function profipaints_customize_preview_js()
{
    wp_enqueue_script('profipaints_customizer_liveview', get_template_directory_uri() . '/assets/js/customizer-liveview.js', array('customize-preview', 'customize-selective-refresh'), false, true);
}
add_action('customize_preview_init', 'profipaints_customize_preview_js', 65);
...
{
    if (!class_exists('Profipaints_Dashboard')) {
        return;
    }

    $actions = Profipaints_Dashboard::get_instance()->get_recommended_actions();
    $number_action = $actions['number_notice'];

    wp_localize_script('customize-controls', 'profipaints_customizer_settings', array(
        'number_action' => $number_action,
        'is_plus_activated' => class_exists('ProfiPaints_Plus') ? 'y' : 'n',
        'action_url' => admin_url('themes.php?page=ft_profipaints&tab=recommended_actions'),
    ));
}
 ...
 */
function profipaints_customize_controls_enqueue_scripts()
{
    wp_localize_script(
        'customize-controls',
        'C_Icon_Picker',
        apply_filters(
            ...
            array(
                'search'    => esc_html__('Search', 'profipaints'),
                'fonts' => array(
                    'font-awesome' => array(
                        // Name of icon
                        'name' => esc_html__('Font Awesome', 'profipaints'),
...
add_action('customize_controls_enqueue_scripts', 'profipaints_customize_controls_enqueue_scripts');
